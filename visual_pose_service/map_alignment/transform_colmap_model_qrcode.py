...
import sys
sys.path.append('/Mobili/python/visual_pose_service')
...
def parse_cameras_txt(path):
    ...
    with open(path, 'r') as f:
        for line in f:
            ...
            cams[cam_id] = {'model': model, 'width': width, 'height': height, 'K': K, 'dist': dist}
    return cams
...
def parse_points3d_txt(path):
    ...
    with open(path, 'r') as f:
        for line in f:
            ...
            pts3d[point_id] = {'xyz': np.array([x, y, z], dtype=float),
                              'rgb': (r, g, b),
                              'error': error,
                              'track': track}
    return pts3d
...
def order_corners_clockwise(pts):
    """
    From four corners in arbitrary order, return stable [tl, tr, br, bl].
    Uses convex hull / centroid, sort by angle (CCW), then map to quadrants.
    """
    arr = np.array(pts, dtype=float)
    if arr.shape[0] != 4:
        # If >4 points (e.g. pyzbar polygon), take hull first
        hull = cv2.convexHull(arr.astype(np.float32)).reshape(-1,2)
        if hull.shape[0] >= 4:
            # Rare: hull has >4 vertices; keep hull vertices
            arr = hull
        else:
    ...
    if any(x is None for x in ordered):
        ...
        ordered = [arr_clockwise[0], arr_clockwise[1], arr_clockwise[2], arr_clockwise[3]]

    return [tuple(map(float,p)) for p in ordered]


def triangulate_qr_corners(records, cams, imgs, qr_id):
    """Multi-view triangulation of QR corners; returns four 3D points."""
    points3d = []
    for corner_idx in range(4):
        ...
        for rec in records:
            ...
            quat = np.array([imgs[rec['image']]['qw'],
                             imgs[rec['image']]['qx'],
                             imgs[rec['image']]['qy'],
                             imgs[rec['image']]['qz']], dtype=np.float64)
            R_cw = quaternion_to_rotation_matrix(quat)
            t_cw = np.array([imgs[rec['image']]['tx'],
                             imgs[rec['image']]['ty'],
                             imgs[rec['image']]['tz']], dtype=np.float64).reshape(3,1)
            P = K @ np.hstack([R_cw, t_cw])  # 3x4 projection matrix

            u,v = rec['corners'][corner_idx]
            # Two rows per observation: u*P[2,:]-P[0,:], v*P[2,:]-P[1,:]
            A.append(u * P[2,:] - P[0,:])
            A.append(v * P[2,:] - P[1,:])
            
            img_name = rec['image']
            if corner_idx == 0:  # log once
                print(f'id: {qr_id}, img: {img_name}, t_cw: {t_cw.flatten()}, t_wc: {(-R_cw.T @ t_cw).flatten()}')

        A = np.asarray(A)
        if A.shape[0] < 4:  # need at least two views
...
def get_qr_center(points3d):
    """QR center from four corner 3D points."""
...
def _init_qr_centers_by_triangulation(all_observations, cams, imgs):
    """Group observations by QR ID, triangulate corners for initial translation."""
    # records: {qr_id: [ {'image':..., 'corners':(4,2)} ]}
    grouped = {}
    for obs in all_observations:
        grouped.setdefault(obs['id'], []).append({'image': obs['image'],
                                                  'corners': obs['corners']})

    init_centers = {}
    init_obj_corners = {}  # local corner coords per QR
...
def estimate_qrcode_poses_global(all_observations, cams, imgs, verbose=2):
    ...
    w_*: loss weights (reserved).
    """
    # All QR IDs
    qr_ids = sorted(set(obs['id'] for obs in all_observations))

    # Local corners; unit square side length 1 in COLMAP scale
    default_half = 0.5
    ...
    for qr_id in qr_ids:
        ...
        if qr_id in init_centers:
            ...
            # Fallback offset if triangulation failed (avoid degenerate zero)
            t0 = np.array([0.0, 0.0, 1.0], dtype=float)  # 1 m in front
        param0.append(np.hstack([rvec0, t0]))
    param0 = np.hstack(param0)

    # Cache (K, dist, R_cw, t_cw) per image for residuals
    view_cache = {}
    for img_name, info in imgs.items():
        ...
        R_cw = quaternion_to_rotation_matrix(quat)        # world -> camera
    ...
    def residuals(p):
        ...
        for obs in all_observations:
            ...
            t_w = qr_params[qr_id][3:].reshape(3,1)    # center in world
            R_wq = rodrigues_to_R(rvec)

            # Four QR corners in world
            ...
            # projectPoints in camera frame with distortion (rvec=0,tvec=0)
            imgpts, _ = cv2.projectPoints(Xc.astype(np.float64),
    ...
    result = least_squares(residuals, param0, method='trf',
                           loss='huber', f_scale=3.0, verbose=verbose,
                           max_nfev=200)
    ...
    # Edge lengths per QR in COLMAP units (~1; noise from reprojection)
    edge_len_stats = {}
    ...
    # Pairwise center distances (COLMAP scale)
    pairwise_dists_est = {}
...
def estimate_scale_from_qr(edge_len_stats, pairwise_dists_est,
                           qr_size_m=None, qr_distances_m=None):
    """
    From optimized COLMAP-scale edge lengths and center distances,
    estimate metric scale using qr_size_m / qr_distances_m.
    ...
    if qr_size_m is not None:
        ...
        if len(means) > 0:
            # Per-QR scale candidate: true edge / estimated edge
    ...
    return {
        'scale_candidates': {'from_size': scales_from_size, 'from_pairs': scales_from_pairs},
        'scale_final': scale_final,
        'used': {'size': len(scales_from_size) > 0, 'pairs': len(scales_from_pairs) > 0}
    }


def estimate_qrcode_poses_optimize(colmap_model_dir, input_sparse_path, image_dir, qr_size_m=None, qr_distances_m=None, output_dir=None):
    """
    Optimize QR poses in COLMAP units, then estimate metric scale.
    """
    ...
    # Global optimize
    qr_poses, edge_len_stats, pairwise_dists_est, init_obj_corners = estimate_qrcode_poses_global(
        all_observations, cams, imgs, verbose=2
    )
    ...
    if output_dir:
        ...
        draw_projection(all_observations, qr_poses, cams, imgs, image_dir,
    ...
    return {
        'qr_poses': qr_poses,
        'edge_len_stats': edge_len_stats,              # COLMAP units
        'pairwise_dists_est': pairwise_dists_est,      # COLMAP units
...
def draw_projection(all_observations, qr_poses, cams, imgs, image_dir, init_obj_corners, save_dir):
    ...
    anchors = {}
    for obs in all_observations:
        anchors.setdefault(obs['id'], []).append({'image': obs['image'],
                                                  'corners': obs['corners']})
    ...
    for qr_id, records in anchors.items():
        ...
        for rec in records[:8]:
            ...
            # Project in camera frame; OpenCV applies distortion
            imgpts, _ = cv2.projectPoints(Xc.astype(np.float64),
                                          np.zeros(3, dtype=np.float64),
                                          np.zeros(3, dtype=np.float64),
                                          K, dist)
            ...
            for i in range(4):
                ...
                cv2.putText(img, str(i), (imgpts[i][0]-8, imgpts[i][1]-8),
                            cv2.FONT_HERSHEY_SIMPLEX, 0.5, (255,0,0), 1, cv2.LINE_AA)

            # Optional: overlay raw detections
...
def apply_sim3_to_colmap_model(colmap_model_dir, input_sparse_path, R, t, scale,
                               output_sparse_path=None, qr_poses=None):
    """
    Apply Sim(3) (scale, R, t) to a COLMAP sparse model and save.

    Args:
        colmap_model_dir: COLMAP project root.
        input_sparse_path: Sparse subdir under colmap_model_dir.
        R: (3,3) rotation.
        t: (3,) translation.
        scale: float scale.
        output_sparse_path: Output subdir (None = overwrite input).
        qr_poses: If set, apply the same Sim(3) to QR poses and write qr_poses.json.
    """
...
def recover_scale(colmap_model_dir, input_sparse_path, qr_poses, scale, output_sparse_path=None):
    ...
    apply_sim3_to_colmap_model(
        colmap_model_dir, input_sparse_path,
        R=np.eye(3), t=np.zeros(3), scale=scale,
...
def align_to_prior_map(colmap_model_dir, input_sparse_path, qr_poses, prior_poses, output_sparse_path=None):
    """
    Sim(3)-align estimated qr_poses to prior_poses and apply to the COLMAP model.
    prior_poses / qr_poses: {qr_id: {'R':(3,3), 't':(3,)}} in world frame.
    """
...
def load_qr_json(qr_json_file):
    if qr_json_file and os.path.isfile(qr_json_file):
        with open(qr_json_file, 'r', encoding='utf-8') as f:
            ...
            qr_distances_m = {
                tuple(k.split(",")): float(v) for k, v in qr_distances_raw.items()
            }
...
if __name__ == '__main__':
    ...
    p.add_argument('--image_dir', default='images', help='directory of input images (same names as in images.txt)')
    p.add_argument('--qr_json_file', default='', help='input json file of qr distances and size')
    p.add_argument('--prior_qr_poses_json_file', default='qr_poses_prior.json', help='input json file of prior qr poses (optional)')
    p.add_argument('--output_dir', default='', help='output dir')
    p.add_argument('--input_sparse_path', default='sparse/0/', help='input sparse model path under colmap_model_dir')
    p.add_argument('--output_sparse_path', default='sparse_test/0/', help='if set, write scaled model to this path instead of overwriting')
    args = p.parse_args()
    ...
    print("used size:", out['scale_info']['used']['size'], " used pairs:", out['scale_info']['used']['pairs'])
    ...
    if args.input_sparse_path == args.output_sparse_path:
        ...
        if args.output_sparse_path is not None:
            recover_scale(args.colmap_model_dir, args.input_sparse_path, out['qr_poses']['anchors'],
                          out['scale_info']['scale_final'], args.output_sparse_path)
